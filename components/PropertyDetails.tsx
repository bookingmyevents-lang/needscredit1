...
const NearbyPlaceIcon: React.FC<{ type: NearbyPlace['type'] }> = ({ type }) => {
    const iconMap: Record<NearbyPlace['type'], React.ReactNode> = {
        School: <Icons.BuildingIcon className="w-5 h-5 text-secondary flex-shrink-0" />,
        Hospital: <Icons.PlusCircleIcon className="w-5 h-5 text-secondary flex-shrink-0" />,
        Restaurant: <Icons.RestaurantIcon className="w-5 h-5 text-secondary flex-shrink-0" />,
        Shopping: <Icons.CreditCardIcon className="w-5 h-5 text-secondary flex-shrink-0" />,
        Park: <Icons.YardIcon className="w-5 h-5 text-secondary flex-shrink-0" />,
        'IT Park': <Icons.BuildingIcon className="w-5 h-5 text-secondary flex-shrink-0" />,
        'Transport Hub': <Icons.MapIcon className="w-5 h-5 text-secondary flex-shrink-0" />,
    };
    return <>{iconMap[type] || <Icons.LocationMarkerIcon className="w-5 h-5 text-secondary flex-shrink-0" />}</>;
};
...
const PropertyDetails: React.FC<PropertyDetailsProps> = ({ properties, users, reviews, property, owner, onBack, onScheduleViewing, onBookNow, onNavigateToHome, onNavigateToBrowsing, currentUser, savedProperties, onToggleSaveProperty, onSelectProperty }) => {
  ...
  const getNearbyPlaceCoords = (place: NearbyPlace, property: Property, index: number): [number, number] | null => {
      if (!hasCoordinates) return null;
      
      const distanceStr = place.distance.split(' ')[0];
      const distanceKm = parseFloat(distanceStr);
      if (isNaN(distanceKm)) return null;

      const R = 6371; // Earth's radius in km
      const angle = (index * (360 / (property.nearbyPlaces?.length || 1))) * (Math.PI / 180);
      const lat1Rad = property.latitude * (Math.PI / 180);
      const lon1Rad = property.longitude * (Math.PI / 180);

      const lat2Rad = Math.asin(Math.sin(lat1Rad) * Math.cos(distanceKm / R) + Math.cos(lat1Rad) * Math.sin(distanceKm / R) * Math.cos(angle));
      const lon2Rad = lon1Rad + Math.atan2(Math.sin(angle) * Math.sin(distanceKm / R) * Math.cos(lat1Rad), Math.cos(distanceKm / R) - Math.sin(lat1Rad) * Math.sin(lat2Rad));

      return [lat2Rad * (180 / Math.PI), lon2Rad * (180 / Math.PI)];
  };
  
  const handleSelectNearbyPlace = (place: NearbyPlace, index: number) => {
    const coords = getNearbyPlaceCoords(place, property, index);
    if (coords) {
        const placeId = `nearby-${index}`;
        setMapCenter(coords);
        setMapZoom(17);
        setSelectedNearbyPlace({ place, coords, id: placeId });
    }
  };
  ...
  const handleMapMarkerClick = (id: string) => {
    if (id.startsWith('nearby-')) {
        ...
        const place = property.nearbyPlaces?.[index];
        if (place) {
            handleSelectNearbyPlace(place, index);
        }
  ...
  const mapMarkers = useMemo(() => {
      ...
      const mainPropertyIcon = L.divIcon({
          ...
          className: 'bg-transparent', iconSize: [16, 16], iconAnchor: [8, 8],
      });
      const nearbyPlaceIcon = L.divIcon({
          html: `<div class="w-3 h-3 bg-secondary rounded-full border-2 border-white shadow"></div>`,
          className: 'bg-transparent', iconSize: [12, 12], iconAnchor: [6, 6],
      });
      
      const markers = [{
          id: property.id,
          position: [property.latitude, property.longitude] as [number, number],
          content: `<b>${property.title}</b>`,
      ...
      (property.nearbyPlaces || []).forEach((place, index) => {
          ...
          if (coords) {
              markers.push({
                  id: `nearby-${index}`,
                  position: coords,
                  content: `<b>${place.name}</b><br/>${place.type} - ${place.distance}`,
  ...
  const availableFromText = () => {
    if (property.availability !== 'available') return 'Rented';
    if (!property.availableDate) return 'Available now';
  ...
  const similarProperties = useMemo(() => {
    if (!properties || !property || !hasCoordinates) return [];
    const RADIUS_KM = 5;
    const PRICE_PERCENT_DIFF = 0.20; // +/- 20%
    return properties
      .filter(p => p.id !== property.id && p.availability === 'available' && p.bedrooms === property.bedrooms && Math.abs(p.rent - property.rent) <= property.rent * PRICE_PERCENT_DIFF && getDistance(property.latitude, property.longitude, p.latitude, p.longitude) <= RADIUS_KM)
      .slice(0, 3);
  }, [properties, property, hasCoordinates]);
    ...
    <>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-2/3">
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2">
              {property.images?.map((img, index) => (
                <img key={index} src={img} alt={`Thumbnail ${index + 1}`} className={`w-24 h-24 object-cover rounded-md cursor-pointer border-2 ${mainImage === img ? 'border-primary' : 'border-transparent'}`} onClick={() => setMainImage(img)} />
              ))}
            </div>
          </div>
          ...
          <InfoCard title="About this property"><p className="text-neutral-700 whitespace-pre-line">{property.description}</p></InfoCard>
          ...
          {safeAmenities.length > 0 && (
            <InfoCard title="Amenities">
                <ul className="grid grid-cols-2 md:grid-cols-3 gap-4 text-neutral-700">{displayedAmenities.map((amenity: Amenity) => (<li key={amenity.name} className="flex items-center gap-3"><AmenityIcon name={amenity.icon} /><span>{amenity.name}</span></li>))}</ul>
                {safeAmenities.length > 8 && (<button onClick={() => setShowAllAmenities(!showAllAmenities)} className="text-primary font-semibold mt-4">{showAllAmenities ? 'Show less' : `+${safeAmenities.length - 8} more`}</button>)}
            </InfoCard>
          )}

          {safeFurnishingItems.length > 0 && (
            <InfoCard title="Furnishing Details">
                <ul className="grid grid-cols-2 md:grid-cols-3 gap-4 text-neutral-700">{safeFurnishingItems.map((item: FurnishingItem) => (<li key={item.name} className="flex items-center gap-3"><AmenityIcon name={item.icon} /><span>{item.name} {item.quantity > 1 ? `(x${item.quantity})` : ''}</span></li>))}</ul>
            </InfoCard>
          )}
                ...
                <div className="space-y-6">
                    {propertyReviews.map(review => (
                        <div key={review.id} className="border-b pb-6 last:border-b-0">
                            <div className="flex items-center gap-3 mb-2"><div className="font-bold text-neutral-800">{review.author}</div><div className="text-xs text-neutral-500">{review.time}</div></div>
                            <div className="flex items-center gap-1 mb-2">{[...Array(5)].map((_, i) => (<Icons.StarIcon key={i} className={`w-5 h-5 ${i < review.rating ? 'text-yellow-400' : 'text-gray-300'}`} />))}</div>
                            <div className="text-sm"><p className="font-semibold text-green-600">What I liked:</p><p className="text-neutral-700">{review.goodThings}</p><p className="font-semibold text-red-600 mt-2">What can be improved:</p><p className="text-neutral-700">{review.needsImprovement}</p></div>
                        </div>
                    ))}
                </div>
          ...
          {owner && (
            <InfoCard title="About the Owner">
                <div className="flex items-center gap-4">
                    ...
                    <div><h4 className="text-lg font-bold">{owner.name}</h4>{owner.kycStatus === 'Verified' && <div className="text-sm font-semibold text-green-600 flex items-center gap-1"><Icons.ShieldCheckIcon className="w-4 h-4" /> Verified Owner</div>}</div>
                </div>
